      ...
      {%- if p.exampleBoolean or p.exampleCanonical -%}
        {%- assign type =  p.reference.reference | split: '/' | first -%}
            {%- if type == i %}
              ...
              {%- if new -%}
                <li><a href="{{p.reference.reference | replace: '/','-'}}"><span class="bg-success" markdown="1">{{p.name}}</span><!-- new-content --></a></li>
              {% else %}
                <li><a href="{{p.reference.reference | replace: '/','-'}}">{{p.name}}</a></li>
              {% endif %}
